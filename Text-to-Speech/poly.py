...
textExample.pack()
def getText():
    aws_mag_con=boto3.session.Session(progile_name='demo_user')
    client = aws_mag_con.client(service_name = 'polly', region_name = 'us-east-1')
    result=textExample.get("1.0","end")
    print(result)
    response = client.synthesize_speech(VoiceId='Aditi',OutputFormat='mp3',Text=result, Engine='neural')
    print(response)
